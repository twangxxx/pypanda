...
class Panda(object):
    ...
    def __expression_data_to_matrix(self):
        ...
        self.gene_names = list(self.expression_data.columns.values)
    ...
    def __motif_data_to_matrix(self):
        ...
        def match(a,b):
            ...
            if a in b:
                return b.index(a)
            return False
        self.unique_tfs = sorted(self.motif_data[self.motif_data.columns[0]])
        #print(self.unique_tfs)
        ...
        #print(self.motif_data[self.motif_data.columns[0]])
        idx_tfs = map(functools.partial(match, b = self.unique_tfs), self.motif_data[self.motif_data.columns[0]])
        #idx_genes = map(functools.partial(match, b = self.gene_names), self.motif_data[1])
        idx_genes = map(functools.partial(match, b = self.gene_names), self.motif_data[self.motif_data.columns[1]])
        
        idx = np.ravel_multi_index((idx_tfs, idx_genes), self.motif_matrix.shape)

        #self.motif_matrix.ravel()[idx] = self.motif_data[2]
        self.motif_matrix.ravel()[idx] = self.motif_data[self.motif_data.columns[2]]
        return None
